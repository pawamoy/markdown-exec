...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    """All public objects in the internal API are exposed under `markdown_exec`."""
    not_exposed = [
        obj.path
        for obj in modulelevel_internal_objects
        if obj.name not in markdown_exec.__all__ or not hasattr(markdown_exec, obj.name)
    ]
...
def test_single_locations(public_api: griffe.Module) -> None:
    ...
    multiple_locations = {}
    for obj_name in markdown_exec.__all__:
...
def test_inventory_matches_api(
    ...
    for item in inventory.values():
        ...
        if (
            ...
            and (item.name == "markdown_exec" or item.name.startswith("markdown_exec."))
        ):
